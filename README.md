...
The blocks` headers in Witnet need to be available to the WBI, meaning the WBI can get those headers that should be stored in some contract, the Block Relay, in Witnet. In order to assure the validity of the blocks' headers, the Block Relay needs to guarantee a consensus protocol and a Finality Gadget, the documentation about the stages of the Block Relay con be found [here][block-relay].
...
This repository contains two Block Relay contracts, the first centralized and the second ABS-based with Finality implemented.

## BlockRelay based on ABS

The `ABSBlockRelay` is a work in progress contract that pretends to implement the Active Bridge Set (ABS) as the set in charge of proposing and posting blocks in the Block Relay. The ABS is updated in the WBI and is a subset of bridge nodes who had achieved enough reputation and so can be considered honest. Please [follow the link][reputation-system] to get further information about the Reputation system in Witnet.

A block header is later finalized when 2/3 of the ABS agreed on a vote. More precisely the flow is as follows:
...
The next step is to integrate a block relay based on the Active Reputation Set (ARS), includind its merklelization as well as the proof of memberhip verification, the Finality Gadget and BLS signatures. More details can be found [here][block-relay].
